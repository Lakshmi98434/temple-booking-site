...
async function loadBrand() {
  ...
  if (res.success && res.data.templeName) {
    ...
    document.getElementById('footerBrand').textContent = res.data.templeName;
    document.title = res.data.templeName;
  }
}
document.getElementById('yearOut').textContent = new Date().getFullYear();
...
function setLang(l) {
  ...
  document.getElementById('btn-kn').classList.toggle('active', l === 'kn');
  document.querySelectorAll('[data-en]').forEach(el => {
    el.textContent = l === 'kn' ? el.getAttribute('data-kn') : el.getAttribute('data-en');
  });
}
